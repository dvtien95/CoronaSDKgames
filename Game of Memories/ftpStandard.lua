...
local imageSet = composer.getVariable("imageSet")
...
local xSize = xPos * 1.75
local ySize = ((15 * HEIGHT) / 16) / heightDivision
...
function scene:create( event )
  ...
  local background = display.newImageRect(sceneGroup, "ftpBG.jpg", 480, 320)
  ...
  local cardSheet = graphics.newImageSheet("ftpStandardCard.png", sheetOption)
  
  local sheetObjectOption = 
  {
    frames = 
    {
      {
  ...
  local tmpImageSelection
  
  if (imageSet == 1) then
    tmpImageSelection = "freeAnimal.png"
  elseif (imageSet == 2) then
    tmpImageSelection = "freeFood.png"
  elseif (imageSet == 3) then
    tmpImageSelection = "freeFruit.png"
  else
    tmpImageSelection = "freeObject.png"
  end
    
  local objectSheet = graphics.newImageSheet(tmpImageSelection, sheetObjectOption)
  ...
  for i = 1, numOfCards, 1 do
    ...
    local tmpImage = display.newImageRect(sceneGroup, objectSheet, assignTable[i], xSize*2.8/4, ySize*2.8/4)
