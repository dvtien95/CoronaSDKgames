...
function scene:create( event )
  ...
  local background = display.newImageRect(sceneGroup, "playground.jpg", 480, 320)
  ...
  local cardSheet = graphics.newImageSheet("standardHardCard.png", sheetOption)
  ...
  local abcORnumber = math.random(1,2)
  
  for i = 1, numOfCards, 1 do
    local tmp1
    if (abcORnumber == 1) then
      tmp1 = math.random(1,99)
    else
      tmp1 = math.random(1,26)
    end
  ...
  for i = 1, numOfCards, 1 do
    ...
    local tmpText
    if abcORnumber == 1 then
      tmpText = display.newText(sceneGroup, assignTable[i], 0, 0, native.systemFont, textSize)
    else
      tmpText = display.newText(sceneGroup, string.char(assignTable[i] + 64), 0, 0, native.systemFont, textSize)
    end
    tmpText:setFillColor(0,0,0)
  ...
  --musicTrack = audio.loadStream( "standardHard.mp3" )
end
